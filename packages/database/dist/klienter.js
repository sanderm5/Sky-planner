...
export async function createKlient(data) {
    ...
    const { data: klient, error } = await client
        ...
        .single();
    if (error)
        throw new Error(`Failed to create klient: ${error.message}`);
    return klient;
}
/**
 * Gets a klient by email
 */
export async function getKlientByEmail(epost) {
    const client = getSupabaseClient();
 ...
 * Gets a klient by ID
 */
export async function getKlientById(id) {
    const client = getSupabaseClient();
    const { data, error } = await client
        .from('klient')
        .select('*')
...
export async function updateKlient(id, data) {
    ...
    const { data: klient, error } = await client
        ...
        .single();
    if (error)
        throw new Error(`Failed to update klient: ${error.message}`);
    return klient;
}
/**
 * Updates password for a klient
 */
export async function updateKlientPassword(id, hashedPassword) {
    const client = getSupabaseClient();
 ...
 */
export async function isEmailRegistered(epost) {
 ...
 */
export async function getKlienterByOrganization(organizationId) {
    const client = getSupabaseClient();
    const { data, error } = await client
        .from('klient')
        .select('*')
        ...
        .order('navn');
    if (error)
        throw new Error(`Failed to get klienter: ${error.message}`);
    return data || [];
}
 ...
 */
export async function getKundeCountByOrganization(organizationId) {
    const client = getSupabaseClient();
    const { count, error } = await client
        ...
        .eq('organization_id', organizationId);
    if (error)
        throw new Error(`Failed to get kunde count: ${error.message}`);
    return count || 0;
}
//# sourceMappingURL=klienter.js.map