 ...
 */
export async function hasActiveSubscription(organizationId: number): Promise<boolean> {
...
export async function logSubscriptionEvent(
  event: Omit<SubscriptionEvent, 'id' | 'processed_at'>
  ...
  if (error) throw new Error(`Failed to log subscription event: ${error.message}`);
  return data;
}
...
export async function updateOnboardingProgress(
  ...
  if (current) {
    ...
    if (error) throw new Error(`Failed to create onboarding: ${error.message}`);
    return data;
  }
}
