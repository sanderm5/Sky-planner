-- ============================================
-- SKY PLANNER - Excel Import System Schema
-- ============================================
-- Run this in Supabase SQL Editor
-- ============================================

-- 1. Import Mapping Templates (Lagrede mappinger per tenant)
-- Must be created first since import_batches references it
...
CREATE POLICY "Import templates org isolation" ON import_mapping_templates
  FOR ALL USING (
    organization_id IN (
      SELECT organization_id FROM klient WHERE id = auth.uid()
      UNION
...
CREATE POLICY "Import batches org isolation" ON import_batches
  FOR ALL USING (
    organization_id IN (
      SELECT organization_id FROM klient WHERE id = auth.uid()
      UNION
...
-- RLS Policies for import_staging_rows
CREATE POLICY "Import staging org isolation" ON import_staging_rows
  FOR ALL USING (
    organization_id IN (
      SELECT organization_id FROM klient WHERE id = auth.uid()
      UNION
...
-- RLS Policies for import_validation_errors (via batch)
CREATE POLICY "Import errors org isolation" ON import_validation_errors
  FOR ALL USING (
    batch_id IN (
      SELECT id FROM import_batches WHERE organization_id IN (
        SELECT organization_id FROM klient WHERE id = auth.uid()
        UNION
        SELECT organization_id FROM bruker WHERE id = auth.uid()
      )
...
-- RLS Policies for import_column_history
CREATE POLICY "Import history org isolation" ON import_column_history
  FOR ALL USING (
    organization_id IN (
      SELECT organization_id FROM klient WHERE id = auth.uid()
      UNION
...
CREATE POLICY "Import audit org isolation" ON import_audit_log
  FOR ALL USING (
    organization_id IN (
      SELECT organization_id FROM klient WHERE id = auth.uid()
      UNION
