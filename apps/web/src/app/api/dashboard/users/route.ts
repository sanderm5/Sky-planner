...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return Response.json(
      { success: false, error: { code: 'ERROR', message: 'Kunne ikke hente brukere' } },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!navn || !epost || !passord) {
      return Response.json(
        { success: false, error: { code: 'ERROR', message: 'Navn, e-post og passord er påkrevd' } },
        { status: 400 }
      );
    }
    ...
    if (!passwordResult.valid) {
      return Response.json(
        { success: false, error: { code: 'ERROR', message: passwordResult.errors[0] } },
        { status: 400 }
      );
    }
    ...
    if (existingUser) {
      return Response.json(
        { success: false, error: { code: 'ERROR', message: 'E-postadressen er allerede registrert' } },
        { status: 409 }
      );
    }
    ...
    return Response.json(
      { success: false, error: { code: 'ERROR', message: 'Kunne ikke opprette bruker' } },
      { status: 500 }
    );
  }
}
