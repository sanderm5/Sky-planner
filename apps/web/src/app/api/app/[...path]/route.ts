...
async function handleProxy(request: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  ...
  // Also decode to catch %2e%2e and other encoded variants
  let decodedPath: string;
  try {
    decodedPath = decodeURIComponent(path);
  } catch {
    return new Response(
      JSON.stringify({ success: false, error: { code: 'BAD_REQUEST', message: 'Ugyldig path-encoding' } }),
      { status: 400, headers: { 'Content-Type': 'application/json' } }
    );
  }
  if (path.includes('..') || path.includes('\0') || decodedPath.includes('..') || decodedPath.includes('\0')) {
    return new Response(
...
// Export individual methods for Next.js to recognize
